...
int main(int argc, char** argv) 
{
    ...
    Sphere testLambert = createSphere(createPoint(0, 0, -5), 1.5, createMaterial(createColor(1, 0, 0), createColor(0,0,0), 0.5));

    addSphereToScene(&scene, testLambert);

    Light testLight = createLight(createPoint(0,3,0), createColor(10,10,10));
    // Light testLight2 = createLight(createPoint(0,0,0), createColor(0,1,0));

    addLightToScene(&scene, testLight);

    // Appel de la fonction de raytracing
    int result = intersectsSphere(rayon, testLambert, &intersect, &t);
